...
//                .build());
    ...
    public void displayGif(ImageView imageView, String imageUri, int size) {
        ...
        //.into(new GlidemipmapImageViewTarget(holder.img, 2));//显示循序播放gif 为2次。默认GIF播放是无限循环的，
    ...
    @Override
    public void displayCirclemipmap(int resId, ImageView imageView) {
        Glide.with(imageView.getContext())
     ...
     * @param imageView
     */
    public void displaymipmap(int resId, ImageView imageView) {
        Glide.with(imageView.getContext())
                .load(resourceIdToUri(imageView.getContext(), resId))
                .centerCrop()
                .crossFade()
